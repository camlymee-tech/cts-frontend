// File: src/previews/HDNTUTPreview.jsx
import { SignatureBlock } from './SignatureBlock';
import { fmtDate } from '../helpers';
import { HDNT_UT_ARTICLES } from '../data/hdntUTArticles';
import { PartyBlock } from './PartyBlock';
import { HDNTClause } from './HDNTClause';

export const HDNTUTPreview = ({ c, seller, customer }) => {
  ...
  return (
    <div className="contract-paper">
      <div className="text-center mb-5">
        ...
        <div className="text-sm italic">(V/v: Ủy thác nhập khẩu hàng hóa)</div>
      ...
      <p className="mb-4 text-sm" style={{ textAlign: 'justify' }}>
        HỢP ĐỒNG NGUYÊN TẮC VỀ VIỆC ỦY THÁC NHẬP KHẨU HÀNG HÓA (sau đây gọi là "Hợp đồng"), được lập {fmtDate(c.date)}, tại Hà Nội giữa Các Bên sau đây:
      </p>

      <PartyBlock heading="BÊN ỦY THÁC (BÊN A)" p={ben_A} />
      <PartyBlock heading="BÊN NHẬN ỦY THÁC (BÊN B)" p={ben_B} />

      <p className="mb-1 text-sm italic">Bên A và Bên B sau đây được gọi riêng là "mỗi Bên" và gọi chung là "các Bên".</p>
      <p className="mb-4 text-sm" style={{ textAlign: 'justify' }}>Sau khi thỏa thuận, các Bên thống nhất ký kết Hợp đồng ủy thác nhập khẩu (sau đây gọi là "Hợp đồng") với các điều khoản và điều kiện như sau:</p>

      {HDNT_UT_ARTICLES.map((article, ai) => (
      ...
      <SignatureBlock marginTop="40px" leftTitle="Đại diện Bên A" leftSub="(Bên Ủy Thác)" rightTitle="Đại diện Bên B" rightSub="(Bên Nhận Ủy Thác)" />
    </div>
  );
};
