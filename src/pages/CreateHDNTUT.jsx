// File: src/pages/CreateHDNTUT.jsx
...
export const CreateHDNTUT = ({ sellers, customers, onSave, setPage, editData, isAdmin = false, saleProfiles = [] }) => {
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => setPage('hdnt_ut')} className="text-blue-600 hover:text-blue-800 text-sm">← Quay lại</button>
        <h1 className="text-2xl font-bold text-gray-800">{isEdit ? '✏️ Sửa HĐ Nguyên Tắc (Ủy thác)' : 'Tạo HĐ Nguyên Tắc (Ủy thác)'}</h1>
      ...
      {step === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="font-semibold text-gray-700 mb-3">Bước 1: Chọn khách hàng (Bên Ủy Thác)</h2>
          {Object.keys(customers).length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              />
              {customerId && <PartyInfoCard title="Thông tin Bên Ủy Thác (tự điền)" p={customer} extra={customer.assignedSale?.code ? <span className="text-gray-400 font-normal"> • Sale: {customer.assignedSale.code}</span> : null} />}
            </div>
          )}
      ...
      {step === 1 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="font-semibold text-gray-700 mb-1">Bước 2: Chọn công ty bên bán (Bên Nhận Ủy Thác)</h2>
          <div className="text-sm text-gray-500 mb-3">Bên Ủy Thác: <strong className="text-gray-700">{customerId} – {customer.companyName}</strong></div>
          {Object.keys(sellers).length === 0 ? (
            ...
            <div className="space-y-3">
              <SearchableSelect
                label="Bên Nhận Ủy Thác (Công ty)" required
              ...
              />
              {sellerId && <PartyInfoCard title="Thông tin Bên Nhận Ủy Thác (tự điền)" p={seller} extra={seller.shortName ? <span className="text-gray-400 font-normal"> • Viết tắt: {seller.shortName}</span> : null} />}
            </div>
          )}
        ...
        <div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-4">
            ...
            <div className="border border-gray-200 rounded-lg p-6 bg-gray-50 max-h-96 overflow-y-auto">
              <HDNTUTPreview c={preview} seller={seller} customer={customer} />
            </div>
          </div>
