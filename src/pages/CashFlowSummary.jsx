...
export const CashFlowSummary = ({ batches = [], customers = {}, onBack }) => {
  ...
  const rows = useMemo(() => {
    ...
    batches.forEach(b => {
      ...
      if (!byCustomer[id]) {
        byCustomer[id] = {
          customerId: id, batchCount: 0, totalAmountVnd: 0, totalDeposit: 0,
          totalDueOnArrival: 0, totalActualCollected: 0, totalRemainingDebt: 0, batchesInDebt: 0,
        };
      }
      ...
      r.totalActualCollected += Number(b.actual_collected) || 0;
      r.totalRemainingDebt += Number(b.remaining_debt) || 0;
      if (Number(b.remaining_debt) > 0) r.batchesInDebt += 1;
    });
  ...
  const grandTotal = filtered.reduce((acc, r) => ({
    ...
    totalActualCollected: acc.totalActualCollected + r.totalActualCollected,
    totalRemainingDebt: acc.totalRemainingDebt + r.totalRemainingDebt,
  }), { batchCount: 0, totalAmountVnd: 0, totalDeposit: 0, totalDueOnArrival: 0, totalActualCollected: 0, totalRemainingDebt: 0 });
        ...
        {filtered.length === 0 ? (
          ...
          <table className="w-full text-sm min-w-[1000px]">
            <thead><tr className="bg-gray-50 text-gray-500 text-xs uppercase">
              ...
              <th className="text-right px-4 py-3">Tổng tiền hàng</th>
              <th className="text-right px-4 py-3">Tổng tiền cọc</th>
              <th className="text-right px-4 py-3">Tổng phải thu khi về</th>
              <th className="text-right px-4 py-3">Tổng đã thu thực tế</th>
              <th className="text-right px-4 py-3">Tổng công nợ còn lại</th>
              <th className="text-right px-4 py-3">Lô còn nợ</th>
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.customerId} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right">{fmtNum(r.totalActualCollected)}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => setPrintCustomerId(r.customerId)} className="text-blue-600 hover:text-blue-800 font-medium">🖨️ In DNTT</button>
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t-2 border-gray-300 font-semibold bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">{fmtNum(grandTotal.totalActualCollected)}</td>
