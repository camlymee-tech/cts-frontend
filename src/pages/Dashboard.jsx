  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <Stat label="HĐ Nguyên Tắc" value={counts.HDNT} color="bg-green-50 text-green-700 border border-green-200" />
        <Stat label="Đơn Đặt Hàng" value={counts.DDH} color="bg-yellow-50 text-yellow-700 border border-yellow-200" />
        <Stat label="Biên Bản BG" value={counts.BBBG} color="bg-purple-50 text-purple-700 border border-purple-200" />
      </div>

      <div className="grid grid-cols-4 gap-3 mb-6">
        <QuickBtn color="bg-green-600" icon="📋" title="Tạo HDNT" sub="Hợp đồng nguyên tắc" page="create-hdnt" />
        <QuickBtn color="bg-yellow-500" icon="📦" title="Tạo DDH" sub="Đơn đặt hàng + AI VAT" page="create-ddh" />
              ...
              {recent.map(c => (
                <tr key={c.contractId} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5">
                    <Badge color={c.type === 'HDNT' ? 'green' : c.type === 'DDH' ? 'yellow' : 'purple'}>{c.type}</Badge>
                  </td>
